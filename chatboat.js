...
async function loadChatHistory() {
  ...
  try {
    const response = await fetch("https://chatserver-2wn6.onrender.com/admin/logs", {
      method: "GET",
      headers: {
    ...
    logs.forEach((line, i) => {
      ...
      if (line.includes("User:") && !sessionStarted) {
        sessionStarted = true;
        const previewText = messagePart.replace("User: ", "").slice(0, 20) + "...";
        ...
        const sessionCopy = [...currentSession]; // Take a safe copy before reset
...
async function sendMessage() {
  ...
  try {
    const response = await fetch("https://chatserver-2wn6.onrender.com/ask", {
      method: "POST",
      headers: {
